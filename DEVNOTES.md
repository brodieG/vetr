...
Need to think about how to reconcile the output of `alike` with the two possible presentations resulting from validate:

* a single error such as: "Argument `current` should be length ..."
* multiple error such as "Argument `current` should meet at least one of the following:"

The error message must make sense for `alike` stand-alone, so it makes sense that the "should" is included in the return value for `alike`.  The problem is that we would like to omit that from the second type of error message, otherwise the first line is saying "should" something, and then each individual item is repeating the admonition.

This is made even more annoying by uneven treatment by disparities across error messages generated by `validate` proper.
