...
learn.  Every other package adds either many functions, or new concepts that
need to be mastered:
...
are limited in their expressive power.  `assertthat` relies primarily<sup><a
href='#fn-1'>1</a></sup> on standard R expressions so is equivalent to
`stopifnot` in this respect.
...
The `vetr` arguments are matched to those of the enclosing function.  We
reference `x` with `.`, but also we must use `&&` instead of `,` to delimit our
checks.  Additionally, `vetr` introduces templates, so we can rewrite
`simple_vetr_a` as:
...
attributes).  Standard Tokens, marked by the `.` symbol, are evaluated as they
would be by `stopifnot`.
...
```{r}
# assertive: many simple Predicate Functions
simple_assertive <- function(num, flag) {
  assert_is_numeric(x)
  assert_is_of_length(x, 2)
  assert_all_are_not_na(x)
}
# checkmate: fewer more flexible/complex Predicate Functions
simple_checkmate <- function(num, flag)
  assertNumeric(num, any.missing=FALSE, len=2)
```

For this type of check the improvements from the third party packages are
...
`stopifnot`, and with similar error messages.  See the code appendix for those
implementations.
...
<sup>1</sup>:<a name='sup-1'></a>
  `assertthat` requires Check Expressions to evaluate to TRUE or FALSE, whereas
  ...
  Predicate Functions for common checks (e.g. scalars, etc.).
