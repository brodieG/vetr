...
We compare the `stopifnot` and the checking packages in the context of function
parameter checks.  We do this by writing a trivial function that does nothing
other than check its parameters.
...
`vetr::all_bw` instead of the semantically equivalent `!anyNA(.) && all(. > 0)`
expression.  `all_bw` is implemented in C and avoids the intermediate vectors
required to evaluate the standard R version.  `checkmate` does the same with
`qassert`.
