...
* [`vetr`](https://github.com/brodieG/vetr) by Yours Truly, augments test
  expressions with extensible template based declarative checks for object
  structure, with an emphasis on informative error messages and speed.
...
`assertthat` hews very close to `stopifnot` so it basically doesn't have any
...
[appendix](#appendix) for `list_funs`):
...
Additionally, in my limited experience the intersection between the checks I
wanted to carry out and the available `assertive` functions was not great.
`checkmate` has more complex check functions which increases the learning curve
further, but allows for some very concise and effective checks.
...
`vetr` introduces the concept of templates, which introduces a learning curve,
but also allows us to express very complex structural requirements succinctly.
The fallback mode evaluates standard R expressions, although there is a special
type of programmable non-standard evaluation that allows you to
transparently construct and compound vetting expressions as R language objects.
