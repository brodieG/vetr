...
* `assertive`, `checkmate`: extensive libraries of Predicate Functions, with
  assertive focusing on many single argument functions, and `checkmate` fewer
  but more flexible and complex functions.
...
missing values.  We illustrate with function that enforce that requirement and
do nothing else.  In `stopifnot`:
...
`vetr` can look similar on the surface:

```{r}
simple_vetr_a <-
  function(x) vetr(is.numeric(.) && length(.) == 2 && !is.na(.))
```

The `vetr` arguments are positionally matched to those of the enclosing
function.  This is why we can reference `x` with `.`, and also why we use `&&`
instead of `,` to delimit our checks.

Additionally, `vetr` introduces the concept of templates, so we can rewrite
`simple_vetr_a` as:
...
`numeric(2L) && !is.na(.)` is a Vetting Expression that contains the Template
Token `numeric(2L)` and the Standard Token `!is.na(.)`.  Template Tokens
require that parameters match their structure (i.e. length, type, and
attributes).  Standard Tokens, distinguished by the presence of the `.` symbol,
are evaluated as they would be by `stopifnot`.

Templates should be familiar to those who have used `vapply`, but there are some
wrinkles.  For example zero-length templates like `numeric()` match any
length objects.  See `?vetr::alike` and `vignette('alike', 'vetr')` for details.

`asserthat` is like `stopifnot`:

```{r}
simple_assertthat <- function(x)
  assert_that(is.numeric(x), length(x) == 2, !is.na(x))
```

`assertive` and `checkmate` both rely on the Predicate Functions they implement,
although the former prefers many simple functions and the latter fewer complex
function:
...
simple_checkmate <- function(num, flag)
  assertNumeric(num, any.missing=FALSE, len=2)
```
