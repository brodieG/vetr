...
* `vetr` requires understanding how to build templates used in structure checks.
...
* `checkmate` provides highly configurable functions, and domain specific
...
* `assertive` is focused on single parameter [Predicate
  Functions](#predicate-function), which by nature are limited in their
  expressive power.
...
Functions](#predicate-function) they implement:
