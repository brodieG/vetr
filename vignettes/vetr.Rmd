...
you'll need to use `....`.  If you have a custom token that does not
reference the vetting object (i.e. does not use `.`) you can mark it as a
custom token by wrapping it in `.()` (if you want to use a literal `.()` you can
use `..()`).
...
will always fail because `logical(1L)` is part of a custom token and is evaluated
