...
The vetting expression is actually the unevaluated call (i.e. the result of
`quote(logical(1L))`), not the object `logical(1L)`.  When you save vetting
expressions to variables you must quote them:

```{r, eval=FALSE}
truefalseorNA <- quote(logical(1L))
vet(truefalseorNA, 1:3)
```

### Parsing Vetting Expressions

Vetting expressions 
combine template tokens and custom tokens delimited by `&&`
and `||`. Custom tokens are unevaluated calls that contain the `.` symbol (e.g.
`. > 0`).  Every other token is consider a template token.
...
substituted and parsed into component tokens.  A token.

Each token is then examined to determine whether it should be treated as a template token or a custom token.
For example:
