...
which will be treated as a single custom validation token instead of a template and a custom validation token.
...
```{r}
TF <- mk_val_token(logical(1L) && !is.na(.), "be TRUE or FALSE")
validate(TF, NA)
```
...
A simple built-in alternative is to use `stopifnot`.  Here are the equivalent checks with `stopifnot`:
