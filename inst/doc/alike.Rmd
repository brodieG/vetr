...
`alike` is written in C to minimize the computational burden of including it in other functions:
...
function, it is competitive., it is competitive and is actually
faster than a bare bones R function that checks types and length.  As objects
grow more complex, `identical` will obviously pull ahead, though `alike` should
be sufficiently fast for most applications:
