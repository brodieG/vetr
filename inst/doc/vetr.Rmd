...
the overhead of adding parameter checks to your functions.
...
The `quote` call prevents `vet` from recursively parsing the rest of the call
and treats `x && y` as a template for a language object.  If we want to store
...
in the same way as `match.call` would.  For example, if we wished to vet just
the third argument:
