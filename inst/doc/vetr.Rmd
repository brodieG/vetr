...
* `vet` recursively checks that the class of the data frame columns (in fact, it
  will recursively check any value or attribute defined in the template)
...
```
This a lot more typing, *and* a lot more thinking.  In fact, while creating this
example we had to keep adding tests that we realized `vet` was carrying out
automatically that we had not thought.
...
data.frame in the template to allow match any number of rows.  See
the [alike vignette](alike.html) for extensive details on how templates work.
