...
#'   \code{vet/vetr/alike}.
#' @param result.list.size how many tokens compounded with `||` we keep track
#'   of, intended mostly as a safeguard in case a logic error causes us to keep
...
#'   `||` compounded tokens, enough so that if we reach that number it is
#'   more likely something went wrong.
...
vetr_settings <- function(
  ...
  env=NULL, result.list.size=1024L
) {
