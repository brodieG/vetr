...
#' strictly greater/less than via the `include.bounds` parameter.
#'
...
#'   `Inf`, and `include.bounds` is \dQuote{[]}.
...
#' all_bw(vec, 0, include.bounds="(]") # All strictly +ve nums
#' all_bw(vec, 0, include.bounds="[)") # All finite +ve nums

all_bw <- function(x, lo=-Inf, hi=Inf, na.rm=FALSE, include.bounds="[]")
