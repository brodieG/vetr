#' Validate Arguments to a Closure
#'
...
# #' Version of \code{`check_args`} That Doesn't Guess at Check Type
# #'
# #' This is primarily useful when you wish to evalaute tests outside of the call
# #' to \code{`\link{check_args}`}, for example:
# #' \preformatted{
# #' function(x, y, z) {
# #'   x.test <- is.integer(x) && all(x > 3)
# #'   y.test <- matrix()
# #'   z.test <- character()
# #'   check_args_exp(templates=list(y=y.test, z=z.test), exp=list(x.test))
# #'   # ... rest of function
# #' }
# #' }
# #' since there is no way for \code{`\link{check_args}`} to know that
# #' that \code{`x.test`} originally involved the variable `x` and thus infer
# #' that a type 2 test was desired there.
# #'
# #' @param templates list a list of templates to use for type 1 checks;
# #'   the names of the list items will be matched against the arguments
# #' @param exp list a list of evaluated expressions to check for truth;
# #'   the names of

# check_args_exp <- function(templates=list(), values=list())
